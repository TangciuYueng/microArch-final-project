...
sudo -u root nohup java -jar EmotionProcessor-1.0-SNAPSHOT.jar  --server.port=8883 >> /tmp/JPWOEJ6CVM.log 2>&1 &

ps -ef | grep emotions-1.0-SNAPSHOT.jar | grep -v grep | awk '{print $2}' | xargs kill -9
sudo -u root nohup java -jar Emotions-1.0-SNAPSHOT.jar  --server.port=8884 >> /tmp/JPWOEJ6CVM.log 2>&1 &
