<template>
    ...
    <v-navigation-drawer expand-on-hover rail color="#F4FFFC">
        ...
        <v-list density="compact" nav>
            ...
            <v-list-item prepend-icon="mdi-heart-multiple" title="偏 好 测 试" value="12" @click="navigateTo('偏好测试')" />
    ...
    <v-card>
        <v-layout>
            ...
            <!-- 显示音乐广场 -->
            <music-listen v-if="page == 2" @search-event="handleSearchEvent" @play-music-event="handlePlayMusicEvent"
                @browsing-personal-homepage="handleBrowsingEvent" @detial-play-list-event="handleDetailPlayList" />
            <!-- 显示音乐室 -->
            <music-room v-if="page == 3" />
            <!-- 显示随笔中心 -->
            <diary v-if="page == 4" @add-diary-event="handleAddDiary"
                @browsing-personal-homepage="handleBrowsingEvent" />
            <!-- 显示设置界面 -->
            <setting v-if="page == 10" @browsing-personal-homepage="handleBrowsingEvent" />
            <!-- 显示偏好测试界面 -->
            <preference v-if="page == 12" />
            <!-- 音乐搜索 -->
            <music-search v-if="page == 5" @browsing-personal-homepage="handleBrowsingEvent"
                @detial-play-list-event="handleDetailPlayList" />
            <add-diary v-if="page == 6" @add-diary-event="handleAddDiarySuccessfully" />

            <img v-if="page == 7" :src="getImgSrc('../assets/retract.svg')" class="retract-button" title="getback"
                @click="page = lastPage" />
            ...
            <!-- 音乐播放器 -->
            <music-player @click="
                lastPage = page == 7 ? lastPage : page;
            page = 7;
            " />

            <personal-homepage v-if="page == 8" :user-id="userId" />
            <play-list v-if="page == 9" :play-list-type="playListType" :play-list-id="playListId" :last-page="lastPage"
                @browsing-personal-homepage="handleBrowsingEvent" @last-page-event="handleLastPageEvent"
                @detial-play-list-event="handleDetailPlayList" />
        </v-layout>
    </v-card>
</template>
...
import preference from "@/views/PreferenceTest.vue";
